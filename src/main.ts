...
function historySnapshot(): HistorySnapshot {
  return {
    elements: cloneState(elements),
    pages: cloneState(pages),
    currentPageIndex,
    nextId,
    globalBackSrcs: cloneState(globalBackSrcs),
    importedFonts: cloneState(importedFonts),
    selectedId,
    view,
  };
}
...
function updateHistoryBtns(): void {
...
function pushUndoSnapshot(snapshot = historySnapshot()): void {
  if (restoringHistory) return;
  if (undoStack.length > 0 && snapshotsEqual(undoStack[undoStack.length - 1], snapshot)) return;
...
function checkpoint(): HistorySnapshot {
  const snapshot = historySnapshot();
  pushUndoSnapshot(snapshot);
  return snapshot;
}

function commitCheckpoint(before: HistorySnapshot): void {
  if (restoringHistory) return;
  if (snapshotsEqual(before, historySnapshot())) {
    const last = undoStack[undoStack.length - 1];
    if (last && snapshotsEqual(last, before)) undoStack.pop();
  }
  updateHistoryBtns();
}

function beginLiveHistory(key: string): void {
  if (liveHistoryKey === key) return;
  if (liveHistoryKey) endLiveHistory();
  checkpoint();
  liveHistoryKey = key;
}

function endLiveHistory(): void {
  if (!liveHistoryKey) return;
  liveHistoryKey = null;
  const last = undoStack[undoStack.length - 1];
  if (last && snapshotsEqual(last, historySnapshot())) undoStack.pop();
  updateHistoryBtns();
}

function restoreSnapshot(snapshot: HistorySnapshot): void {
  restoringHistory = true;
  elements = cloneState(snapshot.elements);
  pages = cloneState(snapshot.pages);
  globalBackSrcs = cloneState(snapshot.globalBackSrcs);
  importedFonts = cloneState(snapshot.importedFonts);
  setImportedFonts(importedFonts);
  currentPageIndex = Math.min(snapshot.currentPageIndex, pages.length - 1);
  nextId = snapshot.nextId;
  page = pages[currentPageIndex];
  selectedId = snapshot.selectedId && elements.some((el) => el.id === snapshot.selectedId) ? snapshot.selectedId : null;
  view = snapshot.view;
...
type SettingsSectionId = "bleed" | "back" | "adjustments" | "project";
type UiPrefs = { settingsSections?: Partial<Record<SettingsSectionId, boolean>> };

const settingsSectionIds: SettingsSectionId[] = ["bleed", "back", "adjustments", "project"];

function loadUiPrefs(): UiPrefs {
  try {
    return JSON.parse(localStorage.getItem(UI_PREFS_KEY) ?? "{}") as UiPrefs;
  } catch {
    return {};
  }
}
...
function saveUiPrefs(prefs: UiPrefs): void {
  try {
    ...
    console.warn("Print layout: UI preference save failed");
  }
}

function wireSettingsSectionPrefs(): void {
  const prefs = loadUiPrefs();
  for (const id of settingsSectionIds) {
    const details = document.getElementById(`settings-section-${id}`) as HTMLDetailsElement | null;
    if (!details) continue;
    const savedOpen = prefs.settingsSections?.[id];
    if (savedOpen !== undefined) details.open = savedOpen;
    details.addEventListener("toggle", () => {
      const next = loadUiPrefs();
      next.settingsSections = { ...(next.settingsSections ?? {}), [id]: details.open };
      saveUiPrefs(next);
    });
  }
}
...
function pageOrientation(ps: PageSettings = page): PageOrientation {
  return ps.widthIn > ps.heightIn ? "landscape" : "portrait";
}

function pagePreset(ps: PageSettings = page): PagePreset {
  const short = Math.min(ps.widthIn, ps.heightIn);
  const long = Math.max(ps.widthIn, ps.heightIn);
  const same = (a: number, b: number) => Math.abs(a - b) < 0.001;
  if (same(short, 8.5) && same(long, 11)) return "letter";
  if (same(short, 8.27) && same(long, 11.69)) return "a4";
  return "custom";
}

function presetDims(preset: PagePreset, orientation: PageOrientation): { widthIn: number; heightIn: number } | null {
  const portrait = preset === "letter"
...
function clampPageElements(ps: PageSettings = page): void {
  for (const el of pageElements(pages.indexOf(ps))) {
    el.x = clamp(el.x, 0, Math.max(0, ps.widthIn - el.widthIn));
    el.y = clamp(el.y, 0, Math.max(0, ps.heightIn - el.heightIn));
  }
}
...
function attachSidebarEvents(): void {
  document.querySelectorAll<HTMLInputElement>(".el-name").forEach((inp) => {
    ...
    inp.addEventListener("change", () => {
      ...
      if (el && el.name !== inp.value) {
        const before = checkpoint();
        el.name = inp.value;
        render();
        commitCheckpoint(before);
      }
    });
  });
  ...
  document.querySelectorAll<HTMLInputElement>(".el-w").forEach((inp) => {
    ...
    inp.addEventListener("change", () => {
      const el = elements.find((x) => x.id === inp.dataset.id);
      const widthIn = Math.max(0.25, parseFloat(inp.value) || 2.5);
      if (el && el.widthIn !== widthIn) {
        const before = checkpoint();
        el.widthIn = widthIn;
        render();
        commitCheckpoint(before);
  ...
  document.querySelectorAll<HTMLInputElement>(".el-h").forEach((inp) => {
    ...
    inp.addEventListener("change", () => {
      const el = elements.find((x) => x.id === inp.dataset.id);
      const heightIn = Math.max(0.25, parseFloat(inp.value) || 3.5);
      if (el && el.heightIn !== heightIn) {
        const before = checkpoint();
        el.heightIn = heightIn;
        render();
        commitCheckpoint(before);
      }
    });
  ...
  document.querySelectorAll<HTMLInputElement>(".el-border-color").forEach((inp) => {
    inp.addEventListener("click", (e) => e.stopPropagation());
    inp.addEventListener("input", () => beginLiveHistory(`el-border-color:${inp.dataset.id}:${inp.dataset.side}`));
    ...
    inp.addEventListener("change", endLiveHistory);
    inp.addEventListener("blur", endLiveHistory);
  });
  ...
  for (const el of visibleEls) {
    ...
    if (view === "front") {
      div.addEventListener("mousedown", (e) => {
        e.preventDefault();
        selectedId = el.id;
        drag = { id: el.id, startMouseX: e.clientX, startMouseY: e.clientY, startElX: el.x, startElY: el.y, before: checkpoint() };
...
function autoLayout(recordHistory = true): void {
  const elems = pageElements();
  if (elems.length === 0) return;
  const before = recordHistory ? checkpoint() : null;

  // 1. Pack rows by trim size. Bleed may extend into inter-card gaps; only the
  // row/page outer edges need additional reserved space.
  const m = AUTO_MARGIN_IN;
  const gap = AUTO_GAP_IN;
  const bleedFor = (el: PrintElement, ps: PageSettings = page) => el.bleedIn ?? ps.bleedIn ?? 0;
  const cardGap = (a: PrintElement, b: PrintElement, ps: PageSettings = page) =>
    Math.max(gap, bleedFor(a, ps) + bleedFor(b, ps));
  const rowGap = (a: number, b: number) => Math.max(gap, a + b);
  const rowOuterWidth = (rowEls: PrintElement[], ps: PageSettings = page) => {
    if (rowEls.length === 0) return 0;
    const trimW = rowEls.reduce((a, el) => a + el.widthIn, 0);
    const gaps = rowEls.slice(1).reduce((a, el, idx) => a + cardGap(rowEls[idx], el, ps), 0);
    return trimW + gaps + bleedFor(rowEls[0], ps) + bleedFor(rowEls[rowEls.length - 1], ps);
  };
  const rowBleed = (rowEls: PrintElement[], ps: PageSettings = page) => Math.max(...rowEls.map((el) => bleedFor(el, ps)));
  const rowTrimHeight = (rowEls: PrintElement[]) => Math.max(...rowEls.map((el) => el.heightIn));
  const sheetOuterHeight = (heights: number[], bleeds: number[]) => {
    if (heights.length === 0) return 0;
    const trimH = heights.reduce((a, h) => a + h, 0);
    const gaps = bleeds.slice(1).reduce((a, b, idx) => a + rowGap(bleeds[idx], b), 0);
    return trimH + gaps + bleeds[0] + bleeds[bleeds.length - 1];
  };
  ...
  for (const el of elems) {
    const neededRow = [...row, el];
    const needed = rowOuterWidth(neededRow);
    if (row.length > 0 && needed > availW) {
      rows.push(row);
      row = [el];
    } else {
      row.push(el);
  ...
  // 2. Row trim heights and outer widths. Bleed only affects sheet perimeter.
  const rowHeights = rows.map((r) => rowTrimHeight(r));
  const rowBleeds = rows.map((r) => rowBleed(r));
  const rowWidths  = rows.map((r) => rowOuterWidth(r));

  // 3. Split rows into sheets when cumulative height exceeds available height
  const availH = page.heightIn - 2 * m;

  type Sheet = { rows: PrintElement[][]; heights: number[]; widths: number[]; bleeds: number[] };
  const sheets: Sheet[] = [{ rows: [], heights: [], widths: [], bleeds: [] }];
  for (let r = 0; r < rows.length; r++) {
    const s = sheets[sheets.length - 1];
    const neededHeights = [...s.heights, rowHeights[r]];
    const neededBleeds = [...s.bleeds, rowBleeds[r]];
    if (s.rows.length > 0 && sheetOuterHeight(neededHeights, neededBleeds) > availH) {
      sheets.push({ rows: [], heights: [], widths: [], bleeds: [] });
    }
    const target = sheets[sheets.length - 1];
    target.rows.push(rows[r]); target.heights.push(rowHeights[r]); target.widths.push(rowWidths[r]); target.bleeds.push(rowBleeds[r]);
  }
  ...
  for (let si = 0; si < sheets.length; si++) {
    ...
    const { rows: sRows, heights: sHeights, widths: sWidths, bleeds: sBleeds } = sheets[si];
    const totalH    = sheetOuterHeight(sHeights, sBleeds);
    ...
    let y = startY + sBleeds[0];
    for (let r = 0; r < sRows.length; r++) {
      const rowEls = sRows[r];
      const rowH   = sHeights[r];
      let x = sharedStartX + bleedFor(rowEls[0], ps);
      for (let ei = 0; ei < rowEls.length; ei++) {
        const el = rowEls[ei];
        el.x = x;
        el.y = y + (rowH - el.heightIn) / 2;
        el.pageIndex = pi;
        const next = rowEls[ei + 1];
        x += el.widthIn + (next ? cardGap(el, next, ps) : 0);
      }
      const nextBleed = sBleeds[r + 1];
      y += rowH + (nextBleed != null ? rowGap(sBleeds[r], nextBleed) : 0);
    }
  }

  renderPageTabs();
  render();
  if (before) commitCheckpoint(before);
}
...
document.addEventListener("mouseup", () => {
  if (drag) commitCheckpoint(drag.before);
  drag = null;
  activeSnapLines = [];
  renderSnapLines();
});
...
function loadImage(id: string, side: "front" | "back", file: File): void {
  ...
  void loadFileIntoElement(elements, id, side, file).then((loaded) => {
    if (!loaded) { commitCheckpoint(before); return; }
    render();
    updateReconnectBtn();
    commitCheckpoint(before);
  });
...
async function applyGlobalBack(src: string, handle?: FileSystemFileHandle | string, recordHistory = true): Promise<void> {
  const before = recordHistory ? checkpoint() : null;
...
const selOrientation = document.getElementById("sel-orientation") as HTMLSelectElement;
...
selPreset.addEventListener("change", () => {
  const dims = presetDims(selPreset.value as PagePreset, selOrientation.value as PageOrientation);
  if (dims) {
    const before = checkpoint();
    page.widthIn = dims.widthIn;
    page.heightIn = dims.heightIn;
    inpW.value = String(page.widthIn);
    inpH.value = String(page.heightIn);
    clampPageElements();
    render();
...
selOrientation.addEventListener("change", () => {
  const nextOrientation = selOrientation.value as PageOrientation;
  const dims = presetDims(selPreset.value as PagePreset, nextOrientation);
  const prevWidth = page.widthIn;
  const prevHeight = page.heightIn;
  const before = checkpoint();
  if (dims) {
    page.widthIn = dims.widthIn;
    page.heightIn = dims.heightIn;
  } else if (pageOrientation(page) !== nextOrientation) {
    [page.widthIn, page.heightIn] = [page.heightIn, page.widthIn];
  }
  inpW.value = String(page.widthIn);
  inpH.value = String(page.heightIn);
  if (page.widthIn !== prevWidth || page.heightIn !== prevHeight) {
    if (pageElements().length > 0) autoLayout(false);
    else render();
    commitCheckpoint(before);
    return;
  }
...
function syncSheetBorder(autoRelayout = false): void {
  const before = liveHistoryKey ? null : checkpoint();
  const sel = selectedElement();
  if (sel) {
...
inpSheetBorderColor.addEventListener("input", () => beginLiveHistory("sheet-border-color-front"));
inpSheetBorderColor.addEventListener("input", () => syncSheetBorder(false));
inpSheetBorderColor.addEventListener("change", endLiveHistory);
inpSheetBorderColor.addEventListener("blur", endLiveHistory);
inpSheetBackBorderColor.addEventListener("input", () => beginLiveHistory("sheet-border-color-back"));
inpSheetBackBorderColor.addEventListener("input", () => syncSheetBorder(false));
inpSheetBackBorderColor.addEventListener("change", endLiveHistory);
inpSheetBackBorderColor.addEventListener("blur", endLiveHistory);
...
function updateAdjContext(): void {
  ...
  if (sel) {
    ...
    contextEl.className = "text-[10px] text-slate-500 -mt-1 mb-1 truncate";
  }
}
...
function wireLiveAdjustmentInput(inp: HTMLInputElement, key: string): void {
  inp.addEventListener("input", () => {
    beginLiveHistory(`adjustment:${key}:${selectedId ?? "sheet"}:${view}`);
    syncAdjustments();
  });
  inp.addEventListener("change", endLiveHistory);
  inp.addEventListener("pointerup", endLiveHistory);
  inp.addEventListener("keyup", (e) => { if (e.key === "Enter" || e.key === "Escape") endLiveHistory(); });
}

wireLiveAdjustmentInput(inpBrightness, "brightness");
wireLiveAdjustmentInput(inpSaturation, "saturation");
wireLiveAdjustmentInput(inpContrast, "contrast");
wireLiveAdjustmentInput(inpBlackpoint, "blackpoint");
wireLiveAdjustmentInput(inpWhitepoint, "whitepoint");
...
document.addEventListener("keydown", (e) => {
  ...
  if (key === "z") {
    ...
    e.preventDefault();
    redo();
  } else if (e.key === "=" || e.key === "+") { e.preventDefault(); stepZoom(0.1); }
