...
export async function saveProject(state: PersistedState, embedImages: boolean): Promise<void> {
  ...
  if (isDesktopApp()) {
    const path = await saveProjectPath("print-layout.printlayout");
    if (!path) return;
  ...
  a.href = url;
  a.download = "print-layout.printlayout";
  a.click();
  ...
  const rawBytes = await new Promise<Uint8Array | null>((res) => {
    if ("showOpenFilePicker" in window) {
      (window as unknown as { showOpenFilePicker: (o: unknown) => Promise<FileSystemFileHandle[]> })
        .showOpenFilePicker({ types: [{ description: "Print Layout Project", accept: { "application/octet-stream": [".printlayout"] } }], multiple: false })
        .then(async ([handle]) => res(new Uint8Array(await (await handle.getFile()).arrayBuffer())))
        .catch(() => res(null));
    } else {
      const input = document.createElement("input");
      input.type = "file";
      input.accept = ".printlayout";
